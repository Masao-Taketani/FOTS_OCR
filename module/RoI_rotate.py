...
class RoIRotate(object):
    ...
    def roi_rotate_tensor(self,
        ...
        with tf.variable_scope("RoIrotate"):
            ...
            crop_boxes = tf.transpose(tf.stack([zeros,
                                                zeros,
                                                norm_box_heights,
                                                norm_box_widths]))
            """
            ...
            rois = tf.image.crop_and_resize(trans_feature_map,
                                            crop_boxes,
                                            box_inds,
                                            crop_size)
    ...
    def roi_rotate_tensor_pad(self,
                              feature_map,
                              transform_matrixs,
                              box_masks,
                              box_widths):
        with tf.variable_scope("RoIrotate"):
            ...
            tile_feature_maps = tf.concat(tile_feature_maps,
                                          axis=0) # N' * H * W * C where N' = N * B
            trans_feature_map = transformer(tile_feature_maps,
                                            transform_matrixs)
            ...
            def body(pad_rois, i):
                ...
                #                                  0,
                #                                  8/map_shape[0],
                #                                  width_box/map_shape[1]]],
                #                                [0],
                #                                [8,
                #                                 tf.cast(width_box, tf.int32)])
                roi = tf.image.crop_to_bounding_box(_affine_feature_map,
                                                    0,
                                                    0,
                                                    8,
                                                    width_box)
    ...
    def roi_rotate_tensor_while(self,
                                feature_map,
                                transform_matrixs,
                                box_masks,
                                box_widths,
        ...
        with tf.variable_scope("RoIrotate"):
            ...
            def body(pad_rois, i):
                ...
                # Project transform
                after_transform = tf.contrib.image.transform(_feature_map,
                                                             matrix,
                                                             "BILINEAR")
                ...
                #                                [[0,
                #                                  0,
                #                                  8/(config.INPUT_IMAGE_SIZE / 4.0),
                #                                  width_box/(config.INPUT_IMAGE_SIZE / 4.0)]],
                #                                 [0],
                #                                 [8, width_box])
                ...
                #                                [[0,
                #                                  0,
                #                                  8/(config.INPUT_SIZE / 4.0),
                #                                 width_box/(config.INPUT_SIZE / 4.0)]],
                #                                [0],
                #                                [8,
                #                                 tf.cast(width_box, tf.int32)])
                roi = tf.image.crop_and_resize(after_transform,
                                               ...
                                               [8,
                                                tf.cast(width_box,
                                                        tf.int32)])
                #  = tf.image.crop_and_resize(after_transform,
                #                             [[0,
                #                               0,
                #                               8/config.INPUT_IMAGE_SIZE,
                #                               width_box/config.INPUT_IMAGE_SIZE]],
                #                             [0],
                #                             [8, width_box])
                # roi = tf.image.crop_and_resize(after_transform,
                #                                [[0,
...
def check_RoIRotate(RR):
    ...
    # pad_rois = RR.roi_rotate_tensor(input_feature_map,
    #                                 input_transform_matrix,
    #                                 input_box_masks,
    #                                 input_box_widths)
    pad_rois = RR.roi_rotate_tensor_pad(input_feature_map,
                                        input_transform_matrix,
                                        input_box_masks,
                                        input_box_widths)
    ...
    with tf.Session() as sess:
        inp_dict = {input_feature_map: data[0],
                    input_feature_height: data[1],
                    input_feature_width: data[2],
                    input_transform_matrix: data[3],
                    input_box_widths: data[5]}
        for i in range(2):
