...
class Recognition(object):
    def __init__(self,
        ...
        self.batch_norm_params = {'decay': 0.997,
                                  'epsilon': 1e-5,
                                  'scale': True,
    ...
    def bilstm(self, input_feature, seq_len):
        with tf.variable_scope("recog/rnn"):
            ...
            lstm_bw_cell = tf.nn.rnn_cell.DropoutWrapper(lstm_bw_cell,
                                                         input_keep_prob=self.keepProb,
                                                         output_keep_prob=self.keepProb)
            infer_output, _ = tf.nn.bidirectional_dynamic_rnn(lstm_fw_cell,
                                                              lstm_bw_cell,
                                                              input_feature,
    ...
    def build_graph(self, rois, seq_len):
        ...
        logits = tf.reshape(lstm_output,
                            [-1, self.rnn_hidden_num * 2]) # (N * T) * 2H

        W = tf.Variable(tf.truncated_normal([self.rnn_hidden_num * 2,
                                             self.num_classes],
                                            stddev=0.1),
                        name="W")
